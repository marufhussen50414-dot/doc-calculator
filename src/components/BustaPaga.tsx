...
import { UNIFIED\_CALCULATOR, searchFields } from '../config/unifiedCalculator';
...
{ id: '2', label: 'চলتی মাসের Imponibile Fiscale Mese', value: '' }

]);

// Detr. Lav. Dipendente Anno অপশন স্টেট (रेडियो বাটন এবং কাস্টম সাম ইনপুট ফিল্ড সহ)

const [detrAnnoMode, setDetrAnnoMode] = useState<'formula' | 'custom'>('formula');
...
const calculator = UNIFIED\_CALCULATOR;
...
const isRoundingField = fieldId === 'arr\_preced' || fieldId === 'arr\_attuale';
...
required = required.filter(id => id !== 'arr\_preced' && id !== 'arr\_attuale');
...
if (outputFieldId === 'tfr\_annuo\_progr' && tfrAnnuoMode === 'custom') {

const hasValue = customDynamicFields.some(f => f.value !== '' && !isNaN(parseFloat(f.value)));

return { valid: hasValue, missing: hasValue ? [] : ['custom\_fields'] };

}

if (outputFieldId === 'imponibile\_fiscale\_anno' && imponibileAnnoMode === 'custom') {

const hasValue = customImponibileFields.some(f => f.value !== '' && !isNaN(parseFloat(f.value)));

return { valid: hasValue, missing: hasValue ? [] : ['custom\_imponibile\_fields'] };

}

if (outputFieldId === 'detr\_lav\_dip\_anno' && detrAnnoMode === 'custom') {

const hasValue = customDetrAnnoFields.some(f => f.value !== '' && !isNaN(parseFloat(f.value)));

return { valid: hasValue, missing: hasValue ? [] : ['custom\_detr\_anno\_fields'] };

}

if (outputFieldId === 'irpef\_lorda\_mese' && irpefLordaMonthlyMode === 'alternative') {

const altFields = ['alt\_irpef\_imp\_sost', 'alt\_detr\_lav\_dip', 'alt\_imposta\_sost'];
...
if (outputField === 'tfr\_annuo\_progr' && tfrAnnuoMode === 'custom') {
...
if (outputField === 'imponibile\_fiscale\_anno' && imponibileAnnoMode === 'custom') {
...
if (outputField === 'detr\_lav\_dip\_anno' && detrAnnoMode === 'custom') {
...
if (outputField === 'irpef\_lorda\_mese' && irpefLordaMonthlyMode === 'alternative') {

const irpefImpSost = parseFloat(String(inputs['alt\_irpef\_imp\_sost'])) || 0;

const detrLavDip = parseFloat(String(inputs['alt\_detr\_lav\_dip'])) || 0;

const impostaSost = parseFloat(String(inputs['alt\_imposta\_sost'])) || 0;
...
delete updated['arr\_preced'];

delete updated['arr\_attuale'];

return updated;

});

if (outputField === 'arr\_preced' || outputField === 'arr\_attuale') {
...
detrAnnoMode={detrAnnoMode}

onDetrAnnoModeChange={setDetrAnnoMode}
...
detrAnnoMode: 'formula' | 'custom';

onDetrAnnoModeChange: (mode: 'formula' | 'custom') => void;
...
const isTfrAnnuoField = outputField === 'tfr\_annuo\_progr';

const isImponibileAnnoField = outputField === 'imponibile\_fiscale\_anno';

const isDetrAnnoField = outputField === 'detr\_lav\_dip\_anno';

const isIrpefLordaMonthlyField = outputField === 'irpef\_lorda\_mese';
...
const isRoundingField = field.id === 'arr\_preced' || field.id === 'arr\_attuale';
...
checked={detrAnnoMode === 'formula'}

onChange={() => onDetrAnnoModeChange('formula')}
...
checked={detrAnnoMode === 'custom'}

onChange={() => onDetrAnnoModeChange('custom')}

className="text-indigo-600 focus:ring-indigo-500"

/>

<span>Alternative Sum (Monthly Sums)</span>
...
value={inputs['alt\_irpef\_imp\_sost'] || ''}

onChange={(e) => onInputChange('alt\_irpef\_imp\_sost', e.target.value)}
...
value={inputs['alt\_detr\_lav\_dip'] || ''}

onChange={(e) => onInputChange('alt\_detr\_lav\_dip', e.target.value)}
...
value={inputs['alt\_imposta\_sost'] || ''}

onChange={(e) => onInputChange('alt\_imposta\_sost', e.target.value)}
...
(!isDetrAnnoField || detrAnnoMode === 'formula') &&
...
UNIFIED\_CALCULATOR.fields.find((f: any) => f.id === fieldId);
